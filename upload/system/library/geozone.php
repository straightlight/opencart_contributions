<?php
class Geozone {
	public function validateGeoZone($registry, $address, $total) {
		...
		if ($this->config->get('payment_alipay_total') > 0 && $this->config->get('payment_alipay_total') > $total) {
			...
			$status = true;
		} elseif ($this->config->get('payment_alipay_geo_address') == 'geo_zones' && !empty($address['country_id'])) {
			...
			if ($country_info && $country_info['status'] && $country_info['postcode_required']) {
				...
				if ($country_implode && $zone_implode) {
					$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('payment_alipay_geo_zone_id') . "' AND (" . implode(" OR ", $country_implode) . ") AND (" . implode(" OR ", $zone_implode) . ")");
		...
		} elseif ($this->config->get('payment_alipay_geo_address') == 'addresses' && !empty($address['postcode']) && !empty($address['country_id'])) {
			$country_info = $this->model_localisation_country->getCountry($address['country_id']);
