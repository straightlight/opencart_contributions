	...
	public function getZoneToGeoZoneLocation($location) {
		...
		foreach ($results as $result) {
			if ($this->config->get('payment_' . $result['code'] . '_status')) {
				// Addresses
				if ($location == $this->config->get('payment_' . $result['code'] . '_location') && $this->config->get('payment_' . $result['code'] . '_location') == 'address') {
		...
		foreach ($results as $result) {
			if ($this->config->get('shipping_' . $result['code'] . '_status')) {
				// Addresses
				if ($location == $this->config->get('shipping_' . $result['code'] . '_location') && $this->config->get('shipping_' . $result['code'] . '_location') == 'address') {
