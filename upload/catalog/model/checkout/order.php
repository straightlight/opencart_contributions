...
class ModelCheckoutOrder extends Model {
	...
	public function getCustomerSearchesByOrders($data = array(), $filter_data = array()) {
		$sql = "SELECT MIN(`o`.`date_added`) AS `date_start`, MAX(`o`.`date_added`) AS `date_end`, MIN(`cs`.`date_added`) AS `search_date_start`, MAX(`cs`.`date_added`) AS `search_date_end`, `o`.`order_id` AS `order_id`, `o`.`ip` AS `order_ip`, `cs`.`category_id` AS `category_id`, `op`.`product_id` AS `product_id`, `cs`.`ip` AS `customer_search_ip`, SUM(`o`.`total`) AS `total`, MIN((SELECT MIN(`op`.`quantity`) FROM `" . DB_PREFIX . "order_product` `op` WHERE `op`.`order_id` = `o`.`order_id`)) AS `min_products`, SUM((SELECT SUM(`op`.`quantity`) FROM `" . DB_PREFIX . "order_product` `op` WHERE `op`.`order_id` = `o`.`order_id`)) AS `products`, MAX((SELECT MAX(`op`.`quantity`) FROM `" . DB_PREFIX . "order_product` `op` WHERE `op`.`order_id` = `o`.`order_id`)) AS `max_products`, SUM((SELECT SUM(`cs`.`products`) FROM `" . DB_PREFIX . "customer_search` `cs` GROUP BY `cs`.`ip`)) AS `searches` FROM `" . DB_PREFIX . "order` `o` LEFT JOIN `" . DB_PREFIX . "order_product` `op` ON (`op`.`order_id` = `o`.`order_id`) LEFT JOIN `" . DB_PREFIX . "customer_search` `cs` ON (`cs`.`customer_id` = `o`.`customer_id`)";
