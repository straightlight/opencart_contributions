...
class ControllerExtensionModuleGeoZoneValidation extends Controller {
	...
	public function validate(&$route, &$args) {
		$query = "SELECT country_id, COUNT(country_id) AS total_countries, zone_id, COUNT(zone_id) AS total_zones FROM " . DB_PREFIX . "zone_to_geo_zone GROUP BY country_id, zone_id HAVING (COUNT(country_id) > 1) AND (COUNT(zone_id) > 1) AND date_modified < DATE_SUB(NOW(), INTERVAL 1 DAY)";
